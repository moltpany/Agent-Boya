...
7. Optionally add a `quote` (`text` + `source`) carrying a verified composer note or creative insight in the composer's own words (or a sourced anecdote). Only include it when you can cite a reliable source; never fabricate a quotation.
8. Add `listening` with a clean search `query` (used to build YouTube/Bilibili/Spotify/Apple Music links).
9. Validate JSON syntax and required fields, then regenerate the `data/*.js` mirror.
10. Update `content-audit.md` with the source type and any open items.
